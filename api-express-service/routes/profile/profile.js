    ...
    let response = {
      ...
      myMentor: [
        ... await mentor.aggregate([
          { $match: { student: myNetworkName } },
          {
            $lookup: {
              from: 'users',
              localField: 'mentor',
              foreignField: 'networkname',
              as: 'details'
            }
          },
          {
      ...
      mySeniorBuddy: [
        ... await seniorBuddy.aggregate([
          { $match: { student: myNetworkName } },
          {
            $lookup: {
              from: 'users',
              localField: 'senior buddy',
              foreignField: 'networkname',
              as: 'details'
            }
          },
      ...
      myJuniorBuddy: [
        ... await seniorBuddy.aggregate([
          { $match: { "senior buddy": myNetworkName } },
          {
            $lookup: {
              from: 'users',
              localField: 'student',
...
router.get('/get-my-chat-list', verifyToken, async (req, res) => {
  ...
  try {
    ...
    for (const v of chatList) {
      ...
      if (doc) response.push({ name: doc._doc['name'], email: doc._doc['email'], role: doc._doc['role'], avatarUrl: doc._doc['avatarUrl'], networkname: v });
    }
